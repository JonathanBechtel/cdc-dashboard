/*
This script is made to populate the tables at the bottom of the page on pantry.html.  The tables that measure
...
function drawAttendanceTable() {
  ...
  query.setQuery('select A, B WHERE C = "Food Pantry" AND D = "Guest"');
...
function drawVolunteerTable() {
  var query = new google.visualization.Query('https://docs.google.com/spreadsheets/d/1lmmpJs2Bz3EfQWExB4KXq_uJWoLlq1PMCahy6w4ipcE/gviz/tq?gid=41319289');
  query.setQuery('select A, B WHERE C="Food Pantry" AND D="Volunteer"');
  query.send(function (response) {
